import { IBaseModalProps, ModalLayout } from "@/components/custom-ui";
import { IDropZoneHandlerProps, useFormDropzone } from "@/hooks";
...
export const DropzoneModal = (props: IDropzoneProps) => {
    const { modalTrigger, handleClose, header, handleInputChange, fileDeleted, children, ...other } = props;
    ...
    return (
        <ModalLayout modalTrigger={modalTrigger} onClose={handleClose}>
            <div className="px-2 md:px-0">
                ...
                <div>
                    <div className={fileUploaded && children ? "block h-full overflow-y-scroll" : "hidden"}>{children}</div>
                    ...
                    >
                        ...
                        <span className="flex flex-col gap-2 text-center text-sm text-[#676767]">
                            {isDragAccept && <span className="text-xs italic text-primary">This File would be Accepted</span>}

                            {isDragReject && <span className="text-xs italic text-[#EF233C]">This File would be rejected</span>}
